...
def create_queue(queue_name):
    _log(f"Creating SQS queue: {queue_name}")
    sqs = boto3.client("sqs", region_name="us-east-1")
    response = sqs.create_queue(QueueName=queue_name)
    url = response["QueueUrl"]
    _log(f"Queue created with URL: {url}")
    return url


@keyword
def send_message(queue_url, message):
    _log(f"Sending message to queue: {message}")
    sqs = boto3.client("sqs", region_name="us-east-1")
...
def receive_messages(queue_url):
    _log("Receiving messages from queue")
    sqs = boto3.client("sqs", region_name="us-east-1")
    resp = sqs.receive_message(QueueUrl=queue_url)
    msgs = resp.get("Messages", [])
    bodies = [m["Body"] for m in msgs]
    _log(f"Messages received: {bodies}")
    return bodies
...
def create_table(table_name):
    _log(f"Creating DynamoDB table: {table_name}")
...
@keyword
def put_item(table_name, item_id, value):
    _log(f"Inserting item into {table_name}: id={item_id}, value={value}")
    dynamodb = boto3.client("dynamodb", region_name="us-east-1")
    dynamodb.put_item(
        TableName=table_name,
        Item={"id": {"S": item_id}, "value": {"S": value}},
    )
    _log("Item inserted")
...
def get_item(table_name, item_id):
    _log(f"Fetching item from {table_name} with id={item_id}")
    dynamodb = boto3.client("dynamodb", region_name="us-east-1")
    resp = dynamodb.get_item(
        TableName=table_name,
        Key={"id": {"S": item_id}},
    )
    value = resp.get("Item", {}).get("value", {}).get("S")
    _log(f"Retrieved value: {value}")
    return value
